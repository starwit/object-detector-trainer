...
from trainer_core.tests.support.pipeline_test_utils import (
    build_args,
    create_minimal_dataset,
    write_params_yaml,
)
from trainer_core.tests.support.ultralytics_stub import StubYOLO
...
def _patch_rtmdet_train(monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("trainer_core.backends.rtmdet.train_backend", _fake_train_backend)
    return run_dir / "weights" / "best.pt"
...
def _patch_rtmdet_reload(monkeypatch: pytest.MonkeyPatch, calls: list[dict[str, str]]) -> None:
    ...
    monkeypatch.setattr("trainer_core.backends.rtmdet.load_rtmdet_baseline", _fake_load_rtmdet_baseline)
