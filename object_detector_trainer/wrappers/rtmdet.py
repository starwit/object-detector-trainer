...
def _extract_prediction_arrays(pred: Any) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if bboxes is None or scores is None or labels is None:
        raise ValueError("RTMDet prediction is missing bboxes, scores, or labels.")
...
def _coco_eval_pycocotools(
    ...
    return CocoEvalResults(precision, recall, map50, map50_95, per_class, macro_f1=macro_f1)
...
class RTMDetModelAdapter:
    """Expose MMDetection RTMDet through the small YOLO-like API used by the pipeline."""
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        if self._class_names:
            ...
            keep = np.array([int(c) in valid_ids for c in labels], dtype=bool)
            bboxes = bboxes[keep]
            scores = scores[keep]
            labels = labels[keep]
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        if imgsz is not None and int(imgsz) != int(self.resolution):
            raise ValueError(
                f"RTMDet adapter resolution is {self.resolution}, "
